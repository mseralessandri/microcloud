...
locals {
  systems = [
    for i, name in var.vm_names : {
      name     = name
      ip       = cidrhost(var.lookup_subnet, var.ip_offset + (i * var.ip_offset))
...
resource "lxd_instance" "microcloud" {
  ...
  device {
    ...
    properties = {
      ...
      "ipv4.address" = cidrhost(var.lookup_subnet, (count.index + 1) * var.ip_offset)
    }
  }
  ...
  dynamic "device" {
    ...
    content {
      ...
      properties = {
        pool   = var.storage_pool
        source = lxd_volume.ceph_disk[length([for i, system in local.systems : i if i < count.index && system.has_ceph])].name
      }
    }
  }
